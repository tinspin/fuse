...
public class Root extends Service {
	...
	public static class Find extends Service {
				...
				if(poll.equals("link")) { // link list
					...
					if(sort.equals("text")) { // full text search
						...
						if(file.exists()) {
							...
							if(remove)
								obj.remove("key");
							String indented = obj.toString(4);
							event.reply().output(indented.length());
							event.output().print(indented);
						}
						else {
							fail(event, full, poll, type, sort, last);
						}
		...
		private void print_list(Event event, String type, List list, long total, boolean remove) throws Exception {
			...
			event.reply().output(builder.length());
			event.output().print(builder.toString());
		}
